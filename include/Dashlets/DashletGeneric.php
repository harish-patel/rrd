...
class DashletGeneric extends Dashlet
{
    ...
    function saveOptions($req)
    {
        ...
        foreach ($this->searchFields as $name => $params)
        {
            ...
            //bug39170 - end
            if ($widgetDef['type'] == 'datetimecombo' || $widgetDef['type'] == 'datetime' || $widgetDef['type'] == 'date')
            { // special case datetime types
                $options['filters'][$widgetDef['name']] = array();
                if (!empty($req['type_' . $widgetDef['name']]))
                { // save the type of date filter
                    $options['filters'][$widgetDef['name']]['type'] = $req['type_' . $widgetDef['name']];
                }
                if (!empty($req['date_' . $widgetDef['name']]))
                { // save the date
